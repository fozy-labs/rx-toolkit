...
const freshSnapshot: unstable_queryV2.TApiSnapshot = {
...
const productsResource = api.createResourceV2<void, ProductList>({
    key: 'products',
    queryFn: async () => {
        fetchCount++;
...
export function Base() {
    const state = productsResource.useResourceV2Agent();
    const [currentFetchCount] = React.useState(() => fetchCount);

    return (
        <Card>
            <CardHeader className="text-xl font-bold">
                📷 Snapshot-гидрация (Query v2)
            </CardHeader>
            <Divider />
