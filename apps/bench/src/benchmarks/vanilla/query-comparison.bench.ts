...
export async function runQueryComparisonBench() {
  ...
  await createBenchmark('Query Comparison: Создание Resource/Query')
    ...
    await createBenchmark('Query Comparison: Подписка на состояние (1000 подписчиков)')
        .add('rx-toolkit Resource', async () => {
            const userResource = createResource({
                queryFn: ({ id }: { id: number }) => mockApi.getUser(id),
                cacheLifetime: 0,
            });
        ...
        .add('RTK Query', async () => {
            ...
            const sub = store.dispatch(api.endpoints.getUser.initiate(1));
