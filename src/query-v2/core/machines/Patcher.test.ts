...
describe("Patcher", () => {
    ...
    it("P6: resolvePatches with pending then aborted (no more pending after)", () => {
        ...
        const aborted = Patcher.createPatch<typeof original>(
            (d) => {
                d.age = 99;
            },
            { name: "Bob", age: 30 },
        ); // patch was made on data after pending
    ...
    it("P7: resolvePatches with pending then committed keeps both", () => {
        ...
        const committed = Patcher.createPatch<typeof original>(
            (d) => {
                d.age = 99;
            },
            { name: "Bob", age: 30 },
    ...
    it("P10: finishPatch commit keeps originalData when pending remain", () => {
        ...
        const patchB = Patcher.createPatch<typeof original>(
            (d) => {
                d.age = 99;
            },
            { name: "Bob", age: 30 },
    ...
    it("P11: multi-patch sequence — commit first, abort second reverts to original", () => {
        ...
        const patch2 = Patcher.createPatch<typeof original>(
            (d) => {
                d.age = 99;
            },
            { name: "Bob", age: 30 },
    ...
    it("P12: abortAllPending aborts all pending patches", () => {
        ...
        const p1 = Patcher.createPatch<typeof original>((d) => {
            d.name = "Bob";
        }, original);
        const p2 = Patcher.createPatch<typeof original>(
            (d) => {
                d.age = 99;
            },
            { ...original, name: "Bob" },
        );
        const p3 = Patcher.createPatch<typeof original>(
            (d) => {
                d.city = "LA";
            },
            { ...original, name: "Bob", age: 99 },
        );
