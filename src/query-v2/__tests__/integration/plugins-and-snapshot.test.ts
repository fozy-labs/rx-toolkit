...
import { ReactHooksPlugin } from "@/query-v2/plugins";
...
describe("Integration: plugins-and-snapshot", () => {
    ...
    it("INT03: ReactHooksPlugin contributes useResourceV2Agent that works via renderHook", async () => {
        ...
        // resource should have useResourceV2Agent contributed by plugin
        expect((resource as unknown as Record<string, unknown>).useResourceV2Agent).toBeTypeOf("function");
    ...
    it("INT04: server capture → serialize → deserialize → client createApi → createResourceV2 → React hook uses hydrated data", async () => {
        // ── Server side ──
        const { queryFn: serverQf, calls: serverCalls } = createControllableQueryFn<TArgs, TData>();
        const serverApi = createApi({ keyPrefix: "app" });
        const serverResource = serverApi.createResourceV2<TArgs, TData>({
            key: "users",
            queryFn: serverQf,
            cacheLifetime: false as never,
        });
        ...
        const clientResource = clientApi.createResourceV2<TArgs, TData>({
            key: "users",
            queryFn: clientQf,
            cacheLifetime: false as never,
        });
        ...
        const { result } = renderHook(() => useResourceV2Agent(clientResource, { id: 1 }));
    ...
    it("INT14: plugin augmentResource receives merged options", () => {
        ...
        api.createResourceV2<TArgs, TData>({
            key: "users",
            queryFn,
        });
