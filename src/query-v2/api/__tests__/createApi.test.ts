...
import type { IPlugin, TApiSnapshot } from "@/query-v2/types";
...
describe("createApi", () => {
    // ── AP01: createApi returns API with createResourceV2, resetAll, getSnapshot ──
    it("AP01: createApi(options) returns API with createResourceV2, resetAll, getSnapshot", () => {
        const api = createApi({ keyPrefix: "app" });

        expect(api).toBeDefined();
        expect(typeof api.createResourceV2).toBe("function");
        expect(typeof api.resetAll).toBe("function");
        expect(typeof api.getSnapshot).toBe("function");
    });

    // ── AP02: createResourceV2 validates unique key ──
    it("AP02: api.createResourceV2(options) validates unique key", () => {
        const api = createApi();
        const { queryFn } = createQueryFn();

        api.createResourceV2<TArgs, TData>({ key: "users", queryFn });
    ...
    // ── AP03: createResourceV2 merges API defaults with resource options (resource overrides) ──
    it("AP03: api.createResourceV2 merges API defaults with resource options", async () => {
    ...
    // ── AP04: createResourceV2 — resource inherits API-level options ──
    it("AP04: api.createResourceV2 — resource inherits API-level options", () => {
        const api = createApi({ keyPrefix: "app" });
        const { queryFn } = createQueryFn();

        // Resource created without explicit keyPrefix — inherits from API
        const resource = api.createResourceV2<TArgs, TData>({ key: "users", queryFn });
    ...
    it("AP08: createApi saves initialSnapshot; createResourceV2 consumes its slice", () => {
    ...
    it("AP08a: createResourceV2 without matching snapshot key — no hydration", () => {
        const snapshot: TApiSnapshot = {
            version: CURRENT_SNAPSHOT_VERSION,
            keyPrefix: null,
            timestamp: Date.now(),
    ...
    // ── AP08b: resetAll deletes _savedSnapshot — subsequent createResourceV2 sees no snapshot ──
    it("AP08b: api.resetAll() deletes _savedSnapshot — subsequent createResourceV2 sees no snapshot", () => {
        const snapshot: TApiSnapshot = {
            version: CURRENT_SNAPSHOT_VERSION,
            keyPrefix: null,
            timestamp: Date.now(),
    ...
    it("AP08c: Snapshot data older than maxSnapshotDataAge triggers auto-invalidation on createResourceV2", async () => {
    ...
    it("AP10: createResourceV2 without key — still works (snapshot limited)", async () => {
        const api = createApi();
        const { queryFn, calls } = createQueryFn();
    ...
    it("AP11: Standalone createResourceV2 accepts standalone-level options", async () => {
        const { queryFn, calls } = createQueryFn();
