...
describe("ReactHooksPlugin", () => {
    ...
    // PL02: plugin.augmentResource called per createResourceV2()
    it("PL02: augmentResource is called per createResourceV2()", () => {
        const plugin = new ReactHooksPlugin();
        const augmentSpy = vi.spyOn(plugin, "augmentResource");

        const api = createApi({ plugins: [plugin] as const });
        api.createResourceV2({ queryFn: () => Promise.resolve("data"), cacheLifetime: false as never });
    ...
    // PL06: ReactHooksPlugin contributes useResourceV2Agent method
    it("PL06: ReactHooksPlugin adds useResourceV2Agent method to resource", () => {
        const plugin = new ReactHooksPlugin();
        ...
        expect((resource as any).useResourceV2Agent).toBeDefined();
    ...
    it("PL08: error in plugin.augmentResource propagates from createResourceV2", () => {
        const badPlugin: IPlugin = {
            name: "BadPlugin",
            install: vi.fn(),
            augmentResource: () => {
