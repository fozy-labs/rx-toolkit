...
describe('Batcher', () => {
    describe('run(fn)', () => {
        ...
        it('resets isLocked after fn throws (try/finally fix)', () => {
            expect(() => Batcher.run(() => { throw new Error('boom'); })).toThrow('boom');
        ...
        it('propagates error from fn upward', () => {
            expect(() => Batcher.run(() => { throw new Error('test-error'); })).toThrow('test-error');
        });

        it('continues working after error', () => {
            expect(() => Batcher.run(() => { throw new Error('fail'); })).toThrow();
