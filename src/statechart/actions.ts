 ...
 * `assign` / `raise` / `cancel` / `log`) carrying the `BUILTIN` brand and their
 * payload; calling one throws. The interpreter core applies `assign` itself
 * and routes `raise` / `cancel` / `log` through the executor.
 *
