...
type BuiltinName = (typeof BUILTIN_IMPORTS)[number];
...
/** Root keys never emitted: `types` is a TypeScript-only inference helper. */
const DROPPED_ROOT_KEYS = new Set(["types"]);
 ...
 * `and([...])`, ...), `types` is dropped, `undefined` values are skipped.
 *
...
export function toXStateSource<TContext extends MachineContext, TEvent extends EventObject, TOutput>(
    ...
    return `import { ${imports.join(", ")} } from "xstate";\n\n${declaration}`;
}
