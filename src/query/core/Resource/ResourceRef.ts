...
export class ResourceRef<D extends ResourceDefinition> implements ResourceRefInstance<D> {
    ...
    patch(patchFn: (data: D["Data"]) => void): ResourceTransaction | null {
        ...
        const updateFn = (data: D["Data"], savedData: D["Data"] | null, transactions: ResourceTransaction[] | null) => {
            isSkipped = false;

            const [newData, patches, inversePatches] = produceWithPatches<D["Data"]>(data, (draft) => patchFn(draft));
