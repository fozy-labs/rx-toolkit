...
import { CommandIdle } from "@/query/core/machines/CommandIdle";
import { CommandLoading } from "@/query/core/machines/CommandLoading";
import type { CommandLink, IPatchHandle, TCommandMachineInstance, TCommandQueryFn } from "@/query/types";
import type {
    ICommandCacheEntryAddedTools,
    ICommandQueryStartedTools,
    TOnCommandCacheEntryAdded,
    TOnCommandQueryStarted,
} from "@/query/types/command-lifecycle.types";
import { Batcher } from "@/signals/base/Batcher";

import { ResourceRef } from "./ResourceRef";

export interface ICommandCacheEntryOptions<TArgs, TResult> {
...
export class CommandCacheEntry<TArgs, TResult> extends CacheEntry<TCommandMachineInstance<TArgs, TResult>> {
    private _queryFn: TCommandQueryFn<TArgs, TResult>;
    private _link: CommandLink<TArgs, TResult>[];
    ...
    constructor(options: ICommandCacheEntryOptions<TArgs, TResult>) {
        super(
    ...
    initiate(args: TArgs): Promise<TResult> {
        ...
        for (const linkDef of this._link) {
            if (linkDef.optimisticUpdate) {
                const ref = new ResourceRef(linkDef.resource, linkDef.forwardArgs(args));
