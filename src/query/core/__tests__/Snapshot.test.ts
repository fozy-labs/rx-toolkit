...
import { CURRENT_SNAPSHOT_VERSION, type TResourceOptions } from "@/query/types";

type TArgs = { id: number };
type TData = { name: string };

function createResource(key: string, overrides?: Partial<TResourceOptions<TArgs, TData>>) {
...
function createResourceMap(...resources: Array<{ key: string; resource: Resource<TArgs, TData> }>) {
    const map = new Map<string, Resource<TArgs, TData>>();
...
describe("Snapshot", () => {
    ...
    // ── SN08: maxSnapshotDataAge — tested at API layer (createResource level)
    it("SN08: maxSnapshotDataAge auto-invalidation is an API-layer concern", () => {
        ...
        // (maxSnapshotDataAge is handled by createApi/createResource)
    ...
    it("SN10: getSnapshot() throws for compare strategy resources", async () => {
        ...
        const resources = new Map<string, Resource<TArgs, TData>>();
