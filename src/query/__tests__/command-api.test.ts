...
import { _createCommand } from "@/query/api/_createCommand";
import { createApi } from "@/query/api/createApi";
import { ReactHooksPlugin } from "@/query/plugins/ReactHooksPlugin";
import type { IPlugin, IPluginContext, TCommandQueryFn, TCommandOptions } from "@/query/types";
...
describe("_createCommand (standalone factory)", () => {
    it("returns ICommand with createAgent and resetCache", () => {
        const command = _createCommand<TArgs, TResult>(createOptions());
...
describe("createApi().createCommand", () => {
    it("returns augmented command", () => {
        const api = createApi();
        const command = api.createCommand<TArgs, TResult>(createOptions());
    ...
    it("augmentCommand is called on plugins that define it", () => {
        ...
        const command = api.createCommand<TArgs, TResult>(opts);
    ...
    it("detects plugin key collisions", () => {
        ...
        expect(() => api.createCommand<TArgs, TResult>(createOptions())).toThrow(/Plugin key collision/);
    });
    ...
    it("default cacheLifetime for commands is 0", () => {
        ...
        api.createCommand<TArgs, TResult>(createOptions());
