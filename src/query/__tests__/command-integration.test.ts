...
import { useCommandAgent } from "@/query/react/useCommandAgent";
...
describe("Integration: Plugin augmentation", () => {
    it("INT-C10: api.createCommand with ReactHooksPlugin → useCommandAgent exists", () => {
        const api = createApi({
            plugins: [new ReactHooksPlugin()] as const,
        });

        const { queryFn } = createControllableCommandQueryFn();
        const command = api.createCommand<TArgs, TResult>({ queryFn });

        // Plugin should contribute useCommandAgent
        expect((command as unknown as Record<string, unknown>).useCommandAgent).toBeTypeOf("function");
    });
    ...
    it("INT-C11: augmentCommand called with command + options", () => {
        ...
        const { queryFn } = createControllableCommandQueryFn();
        const options: TCommandOptions<TArgs, TResult> = { queryFn };
...
describe("Integration: Concurrent triggers", () => {
    it("rapid-fire triggers — only last one resolves, previous ones abort", async () => {
        ...
        const command = api.createCommand<TArgs, TResult>({ queryFn: commandQf });
        const agent = command.createAgent();
...
describe("Integration: useCommandAgent React hook", () => {
    ...
    it("RH04: stable trigger reference across re-renders", async () => {
        ...
        const { result, rerender } = renderHook(() => useCommandAgent(command));
    ...
    it("RH06: unmount while loading — no console errors", async () => {
        ...
        const { result, unmount } = renderHook(() => useCommandAgent(command));
