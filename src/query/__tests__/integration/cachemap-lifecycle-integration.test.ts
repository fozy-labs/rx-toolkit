...
import { getSnapshot } from "@/query/core/Snapshot";
import * as stableStringifyModule from "@/query/lib/stableStringify";
...
describe("Integration: CacheMap + Lifecycle + Devtools key", () => {
    ...
    it("IT04: resetCache settles all lifecycle resolvers and empties cache", async () => {
        ...
        (resource as unknown as Resource<TArgs, TData>).resetCache();
        await flushMicrotasks();
    ...
    it("IT05: serialize-strategy snapshot entries keyed by serialized args via cacheValues + argsKey", async () => {
        ...
        const resourceInternal = resource as unknown as Resource<TArgs, TData>;
    ...
    it("IT07: entry.argsKey is accessible on IResourceCacheEntry and matches devtools key", async () => {
        const { queryFn, calls } = createControllableQueryFn<TArgs, TData>();
        const resource = _createResource<TArgs, TData>({
            key: "test",
            queryFn,
