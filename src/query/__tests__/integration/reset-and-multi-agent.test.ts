...
describe("Integration: reset-and-multi-agent", () => {
    ...
    it("INT10: resetAll resets all resources — agents see idle, caches empty", async () => {
        ...
        const r1 = api.createResource<TArgs, TData>({ key: "users", queryFn: qf1, cacheLifetime: false });
        const r2 = api.createResource<TArgs, TData>({ key: "posts", queryFn: qf2, cacheLifetime: false });
