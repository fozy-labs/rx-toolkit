...
function createMockResource(overrides?: Partial<IResource<any, any>>): IResource<any, any> {
    return {
...
function createMockEntry(overrides?: Partial<IResourceCacheEntry<any, any>>): IResourceCacheEntry<any, any> {
    return {
        createPatch: vi.fn().mockReturnValue(null),
        invalidate: vi.fn(),
        ...overrides,
    } as unknown as IResourceCacheEntry<any, any>;
}

describe("CommandCacheEntry", () => {
    // ── T30: Extends CacheEntry — peek().status === "idle" initially ──
    it("T30: starts in idle state", () => {
        const { queryFn } = createControllableQueryFn();
        const entry = new CommandCacheEntry({ queryFn });
    ...
    it("T35: onQueryStarted fires and $queryFulfilled resolves on success", async () => {
        ...
        const entry = new CommandCacheEntry({
            queryFn,
    ...
    it("T38: linked resource is invalidated on success when invalidate: true", async () => {
        ...
        const entry = new CommandCacheEntry({ queryFn, link });
