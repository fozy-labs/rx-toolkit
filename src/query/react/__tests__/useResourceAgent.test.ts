...
import { SKIP } from "@/query/lib/SKIP_TOKEN";
...
describe("useResourceAgent", () => {
    ...
    it("RH05: same args on rerender does not trigger re-fetch", async () => {
        ...
        const args = { id: 1 };
        const { rerender } = renderHook(() => useResourceAgent(resource, args));
    ...
    it("RH06: void args resource works without second argument", async () => {
        ...
        const resource = _createResource<void, { items: string[] }>({
            queryFn,
            cacheLifetime: false as never,
        });

        const { result } = renderHook(() => useResourceAgent(resource));
    ...
    it("RH07: multiple hooks sharing same resource/args trigger a single fetch", async () => {
        ...
        const { result: result1 } = renderHook(() => useResourceAgent(resource, { id: 1 }));
        const { result: result2 } = renderHook(() => useResourceAgent(resource, { id: 1 }));
    ...
    it("RH10: rapid unmount/remount produces clean state", async () => {
        ...
        // First mount
        const { unmount } = renderHook(() => useResourceAgent(resource, { id: 1 }));
        unmount();
