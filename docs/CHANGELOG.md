...
### Fixed
- Кросс-табовая синхронизация не работала целиком: вкладка-владелец данных никогда не отвечала на REQ — обработчик передавал уже сериализованный ключ записи в `Resource.getEntry()`, который сериализовал его повторно, и запись не находилась. Каждая холодная запись в остальных вкладках ждала полный таймаут (150 мс) и уходила в `queryFn`. Поиск по входящему ключу переведён на новый `Resource.getEntryByKey(key)` — прямой lookup по сериализованному ключу, без повторной сериализации.
