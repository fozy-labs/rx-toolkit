...
- **Автоочистка `LocalSignal`** — слоты, которые не читались и не писались дольше `maxUnreadTime` (по умолчанию 60 дней), периодически удаляются: примерно раз в `checkInterval` (неделя), без межвкладочных блокировок — случайное смещение запуска (`randomOffset`, до часа) плюс перепроверка дедлайна в мете. Чтение обновляет отметку свежести не чаще раза в `checkInterval`, поэтому живые значения не протухают. Настройки: опция `gc` у `LocalSignal.state(...)` (`false` — слот вечный, `{ maxUnreadTime }` — своё время жизни), глобальные `LocalSignal.GC_OPTIONS` (`syncLimit` / `checkInterval` / `randomOffset`, всё в мс), дефолты — `LOCAL_STATE_GC_DEFAULTS`. Кастомному драйверу для очистки нужно перечисление ключей (`keys()` либо `length`/`key(i)`); без него он работает как раньше, просто без GC.

### Changed
- **BREAKING: формат хранения `LocalSignal` / `LocalState`** — каждый слот (`common` и каждый `userId`) теперь лежит в отдельном ключе хранилища (`__LSValue__:{key}` и `__LSValue__:{key}:user:{id}`) в конверте с метаданными `{ at, ttl?, data }`. Голый ключ `__LSValue__` содержит мету неймспейса: версию формата и план автоочистки. Старый формат не мигрируется: при первом запуске неймспейс `__LSValue__*` очищается и размечается заново; более новая версия формата (соседняя вкладка с новой версией пакета) не трогается.
