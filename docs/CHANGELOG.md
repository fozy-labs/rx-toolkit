...
### Changed
- 💥 **Breaking.** `trigger` на уровне агента и хука (`CommandAgent.trigger`, `useCommand`) снова возвращает **нативный** `Promise<TData>`: данные при успехе, реджект нормализованной через `mapError` ошибкой при провале. Конверт удалён. Реджект пред-обработан внутри агента (внутренний no-op catch), поэтому fire-and-forget вызов (`onClick={() => trigger(args)}`) не порождает unhandled rejection — ошибка по-прежнему отражается в состоянии. Обработка ошибок — обычный `try/catch`; проверка `result.status` больше не работает. См. [гайд по миграции](./migrations/0.11.0.md).
- Императивный API ресурса **стабилизирован**: с `ensure` / `fetch` / `prefetch` снята пометка `@experimental`. `prefetch` получил опции `{ force?: boolean }`: `force: true` — fire-and-forget аналог `fetch` (прогрев заведомо свежими данными; существующая запись перезапрашивается, упавшая ретраится).
- 💥 **Breaking.** Формат хранения `LocalSignal` / `LocalState` — каждый слот (`common` и каждый `userId`) теперь лежит в отдельном ключе хранилища (`__LSValue__:{key}` и `__LSValue__:{key}:user:{id}`) в конверте с метаданными `{ at, ttl?, data }`. Голый ключ `__LSValue__` содержит мету неймспейса: версию формата и план автоочистки. Старый формат не мигрируется: при первом запуске неймспейс `__LSValue__*` очищается и размечается заново (при драйвере без перечисления ключей — точечно при обращении); более новая версия формата (соседняя вкладка с новой версией пакета) не трогается — принадлежность перепроверяется перед каждой разрушающей операцией. Двоеточия в `key` / `userId` экранируются в имени ключа хранилища, чтобы `key: "a:user:b"` не коллидировал с `key: "a", userId: "b"`.
...
- **Автоочистка `LocalSignal`** — слоты, которые не читались и не писались дольше `maxUnreadTime` (по умолчанию 60 дней), периодически удаляются: примерно раз в `checkInterval` (неделя), без межвкладочных блокировок — случайное смещение запуска (`randomOffset`, до часа) плюс перепроверка дедлайна в мете. Отметка свежести обновляется при чтении из хранилища и периодически для «живых» слотов (в сессии создан инстанс; частота ре-touch подстраивается под самый маленький TTL среди живых), поэтому активно используемые значения не протухают даже в долгоживущих сессиях и не сметаются GC других вкладок. Настройки: опция `gc` у `LocalSignal.state(...)` (`false` — слот вечный, `{ maxUnreadTime }` — своё время жизни), глобальные `LocalSignal.GC_OPTIONS` (`checkInterval` / `randomOffset` — мс, `syncLimit` — ключей за синхронную порцию), дефолты — `LOCAL_STATE_GC_DEFAULTS`. Кастомному драйверу для очистки (и для wipe старого формата) нужно перечисление ключей (`keys()` либо `length`/`key(i)`); без него он работает как раньше — без GC, а записи неизвестного формата вычищаются точечно при обращении. Best-effort записи (self-heal, touch, инициализация меты) не ломают чтение и конструктор при отказе хранилища (квота, приватный режим).
...
### Removed
- 💥 **Breaking.** Типы `TTriggerResult` / `TTriggerPromise` и хелпер `wrapTrigger` — удалены вместе с конвертом.
- 💥 **Breaking.** `getDevtoolsKey` — опция ресурса удалена как нерабочая: она никогда не читалась. Передача опции теперь ошибка типизации; поведение DevTools не менялось.
