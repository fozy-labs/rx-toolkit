...
### Added
- **`retentionTime` функцией** — опции `createResource`, `createCommand`, `unstable_createProjectionResource` и уровня API (`resourceRetentionTime`, `commandRetentionTime`) принимают `(args, state) => number | false`, а не только число или `false`. Функцию вызывает переход записи `active → retention` — синхронно внутри отписки последнего подписчика; результат действует на один цикл удержания: новый подписчик отменяет таймер, а следующая потеря подписчиков вызывает функцию заново, с актуальным `state`. `state` — состояние записи без варианта `idle`: `TResourceEntryState` у ресурса, новый публичный `TCommandEntryState` (состояние сцепления команды без `retry()`) у команды. Опция ресурса или команды заменяет опцию API целиком — как и со статическим значением. Статическое значение работает как прежде. См. [docs/query/concepts/cache](./query/concepts/cache.md#время-удержания-записи).
...
- 💥 **Breaking. Опция записи кэша `initialMachine` → `initialState`.** Принимает плоскую запись `TQueryEntryState<TArgs, TData>`; семантика прежняя — переданное состояние подавляет автоматический первый запуск, кроме статуса `invalidating`.
...
### Fixed
- **`resource.getState(args)` больше не трогает таймер удержания.** Синхронное чтение состояния записи не является подписчиком: раньше оно проходило через разделяемый поток записи, а значит подписывалось и отписывалось — и тем самым перезапускало отсчёт `retentionTime` у записи без подписчиков. Ненаблюдаемая запись, которую периодически опрашивали через `getState`, могла из-за этого жить вечно. Теперь read-only чтения (`getState`, проверки статуса внутри `ensure` / `fetch`, проекционный рантайм, SWR-фолбэк сцепления) читают запись напрямую и цикл удержания не начинают — в том числе не вызывают `retentionTime`-функцию. Это про само чтение: `ensure` / `fetch` / `prefetch` по-прежнему подписываются на запись до резолва и, отпуская подписку, начинают обычный цикл удержания — так же, как и со статическим значением. См. [docs/query/concepts/cache](./query/concepts/cache.md#момент-вычисления).
