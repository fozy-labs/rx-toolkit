...
### Fixed
- Стриминговые запросы: ложное однократное предупреждение о патчах при открытом стриме (см. `allowStreamPatches`). Если стримовый запуск прерывался прямо на синхронной первой эмиссии и его вытеснял обычный (промисовый) запуск `queryFn`, внутренний флаг «стрим открыт» зависал навсегда — и каждый последующий `createPatch` на записи выводил предупреждение, хотя стрима уже не было.
- Batch-ресурс: ошибка обёрнутого ресурса проходила через `mapError` API дважды. Теперь она нормализуется ровно один раз — запись набора отдаёт тот же экземпляр ошибки, что и запись обёрнутого ресурса.
- Batch-ресурс: `refresh`, чей ответ перестал покрывать какой-то из запрошенных id, молча отдавал устаревший элемент из кэша. Теперь непокрытые ответом id проваливают запись набора с `BatchItemMissingError` (состояние `refresh-error`, устаревшие данные сохраняются — как у обычного проваленного рефреша).
- Batch-ресурс: `refresh` мог присоединиться к запросу, начатому до него, и завершиться данными «до рефреша». Теперь рефреш всегда выпускает свежие запросы по всем id набора; запуски, начатые после него, присоединяются уже к его запросам.
- Statechart: конфиг, в котором `target` history-ноды по умолчанию резолвился в другую history-ноду (само-таргет, цепочка или взаимный цикл вида `h1 → h2 → h1`), приводил к `RangeError: Maximum call stack size exceeded` при `send()`. Теперь такой конфиг отклоняется уже в `createMachine` с `MachineConfigError` (путь `states.<нода>.target`). Валидация сознательно строже XState: цепочки history → history отвергаются целиком, включая нецикличные.
- Statechart: утечка памяти в подборе переходов — списки кандидатов кэшировались на каждую пару «нода × тип события» без ограничения и вытеснения, так что динамически именуемые события (например, `item.<id>.updated`) и проверки `can()` навсегда оседали в памяти определения машины. Кэш убран (как в XState v5, где его тоже нет): кандидаты считаются заново при каждом вызове; побочное следствие — повторные вызовы больше не возвращают референсно тот же массив (содержимое и порядок не изменились).
- `useInfiniteResource`: склеенный `state.data` пересоздавался (с квадратичной склейкой) на каждой эмиссии состояния — включая чистые смены статуса (success → refreshing) без изменения данных, — из-за чего мемоизированные и виртуализированные списки, завязанные на идентичность `state.data`, перерисовывались на каждом тике фонового рефреша. Теперь `state.data` идентично-стабилен: новый массив создаётся, только когда фактически изменились данные какой-либо страницы, а сборка линейна по числу элементов.


## [0.12.0-rc.2] - 2026-08-31

Заменяет `0.12.0-rc.0` и `0.12.0-rc.1`.
...
- **`api.createBatchResource`** — обёртка над ресурсом для загрузки коллекций по списку id с кэшем на уровне отдельных элементов, для дедупликации и бесканальной загрузки. См. [docs/query/usage/batch-resource](./query/usage/batch-resource.md).
...
### Changed
- **BREAKING** — контракт переходов машины запроса (`Machine` / `MachineBase` / `MachineSuccess` экспортируются из корня пакета): `fail()` из состояния `success` теперь переводит машину в `refresh-error` с сохранением данных, а не бросает `MachineTransitionError`, как в `0.11.2`. Изменение нужно стриминговым запросам — стрим может упасть уже после доставки данных. Также у `success` появился переход `next(data)`: `success → success`, обновление данных на месте очередной эмиссией стрима (с переигрыванием активных патчей). Актуальная диаграмма — в [docs/query/concepts/machine](./query/concepts/machine.md#диаграмма-переходов).
- **BREAKING** — `TQueryStartedContext` (контекст `onQueryStarted`, экспортируется из корня пакета) получил **обязательное** поле `$queryStream: TQueryStreamContext<TData>`. Код, конструирующий этот контекст вручную (тест-дублёры, собственные Resource-подобные реализации), не скомпилируется, пока не добавит поле; для потребителей контекста поле — просто новая возможность (см. [стриминговые запросы](./query/usage/stream-query.md#лайфцикл-хук-querystream)).
...
[0.12.0-rc.2]: https://github.com/fozy-labs/rx-toolkit/compare/v0.11.2...v0.12.0-rc.2
