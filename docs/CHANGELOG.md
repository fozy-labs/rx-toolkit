...
### Added
- Переходы состояния ресурсов и команд помечаются именем действия в devtools: `UPDATE: success`, `UPDATE: error`, `UPDATE: refresh`, `UPDATE: rebase`, `UPDATE: refresh-error`, `UPDATE: retry`, `UPDATE: patch`, `UPDATE: patch-settled`, `UPDATE: sync`. Раньше все обновления кэша приходили безымянным `UPDATE`, и отличить старт запроса от прихода данных можно было только по `status` внутри состояния. Таблица имён — в [docs/devtools](./devtools/README.md#имена-действий-у-ресурсов-и-команд).

### Changed
- Redux Devtools больше не ругается на повторное использование ключа при создании состояния. Каждый вызов `DevtoolsLike.state()` — отдельный инстанс-владелец ключа: если ключ ещё занят прошлым инстансом, запись просто переходит к новому и отправляется действие `RECREATE` вместо `CREATE`. Предупреждение о коллизии теперь выдаётся только тогда, когда она реальна — когда в занятый ключ продолжает писать вытесненный инстанс (одно предупреждение на инстанс, само обновление игнорируется, чтобы дерево показывало текущего владельца). Практический эффект: вызывать `dispose()` у сигналов ради чистоты devtools больше не требуется. Публичные типы `DevtoolsLike` / `DevtoolsStateLike` и `combineDevtools` не изменились — учёт владельцев целиком внутри драйвера.

### Fixed
- Запоздалая очистка старого сигнала стирала запись нового с тем же ключом: `State` при сборке мусора шлёт в devtools `$COMPLETED` через `FinalizationRegistry`, и это удаляло из дерева то, что лежало по ключу на момент GC, — то есть данные уже пересозданного сигнала. Событие завершения от вытесненного инстанса теперь игнорируется.
- Документация devtools приведена в соответствие с кодом: имя сигнала задаётся опцией `key` (не `name`), ресурсы и команды — тоже `key` (несуществующей опции `devtoolsName` больше нет в примерах), отключение devtools у ресурса через `devtoolsName: false` убрано как невозможное.
- Тип `beforeDevtoolsPush` в опциях кэша query (`ICacheEntryOptions`, `IQueryCacheEntryOptions`) не совпадал с реальным контрактом: он был объявлен как `(state) => state`, тогда как хук вызывается с `(value, push, actionName)` и ничего не возвращает. Функция, написанная по объявленному типу, молча перестала бы отправлять состояние в devtools. Оба типа теперь ссылаются на `TBeforeDevtoolsPushFn` из сигналов.
- `push`, передаваемый в `beforeDevtoolsPush`, принимает вторым аргументом `actionName` — до этого имя доходило до хука, но объявленный тип `push` его не принимал, так что переслать его дальше было невозможно.
...
- Имя действия в Redux Devtools могло попасть на чужую запись: при батчинге все обновления батча уходят одной отправкой, а имя и тип действия хранились по одному на соединение — поэтому имя от одного сигнала помечало отправку, в которой ехало обновление совсем другого, а батч с созданием записи целиком помечался `CREATE`. Драйвер теперь ведёт учёт по ключам и собирает тип отправки из всего, что в неё попало: типы склеиваются через `+` в фиксированном порядке (`CREATE+UPDATE`), имена перечисляются через запятую (`UPDATE: refresh, patch`) с обрезкой до пяти и хвостом `+N more`. Для одиночного ключа — подавляющее большинство отправок — формат не изменился.
