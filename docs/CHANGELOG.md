...
Breaking-релиз модуля Query: словарь (Agent → Clutch, `refresh` → `invalidate`, `pack` → `bind`) и форма реактивного состояния (матрица `status` × `dataSource`). Переименования живут алиасами до 0.14.0, форма состояния меняется без них.
...
- 💥 **Breaking. `refresh` → `invalidate` на всех уровнях.** `resource.invalidate(args)`, `entry.invalidate()`, `clutch.invalidate()`, `useInfiniteResource(...).invalidate()`. Статусы машины `refreshing` / `refresh-error` → `invalidating` / `invalidate-error` вместе с классами, типами состояний и методом `machine.invalidate()` — **без алиасов**: у машины меняется форма состояния. `invalidate-error` — провал перезапроса после инвалидации, сама инвалидация не проваливается. См. [docs/query/concepts/machine](./query/concepts/machine.md).
...
- статусы `refreshing` / `refresh-error` — из состояния сцепления и записи (в машине они переименованы в `invalidating` / `invalidate-error`);
- поле `isRetrying` — из состояний машины `TPendingState` / `TInvalidatingState`: повтор в полёте — это `error !== null`;
