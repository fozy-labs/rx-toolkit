...
### Added
- **`unstable_KeyedSignal` (experimental)** — реактивная keyed-коллекция: чтение со скоростью `Map`, запись за O(1) и точечная реактивность по каждому ключу. `get$(key)` подписывает вызывающего ровно на один ключ (будится при добавлении/удалении/замене именно его значения), `values$()` отслеживает структуру (add/remove ключей, но не замену значения существующего). Пер-ключевые узлы материализуются лениво и собираются, когда ключа больше нет и его никто не наблюдает (память следует за живым/наблюдаемым множеством, а не за всеми когда-либо затронутыми ключами); нереактивные чтения (`get`, `has`, `values`, `snapshot`, `size`) узлов не создают, `snapshot()` мемоизируется между записями. Пришёл на смену внутреннему `CacheMap` в слое запросов (`Command`, `Resource`): O(1)-запись вместо O(N)-копии корня у `ProxySignal` и точечная инвалидация вместо пробуждения всех читателей на любое изменение кэша. API помечен как экспериментальный и может измениться до стабилизации.
- **`unstable_ProxySignal` (experimental)** — глубокий реактивный стор поверх графа сигналов. Дерево состояния (массивы + обычные объекты) отображается в ленивое дерево пер-путевых сигналов: `ps.root.a.b()` подписывается ровно на этот путь, а не на весь стор. Запись — через copy-on-write черновик (`ps.mutate((d) => { d.a.b = v })`) или замену целиком (`ps.set(next)`); будятся только затронутые пути, с дедупликацией по `Object.is`. Итерация (`in` / `Object.keys`) реагирует только на добавление/удаление ключей. Узлы создаются лениво и собираются, когда на них не остаётся подписчиков (нет утечки при ротации ключей). `Map`/`Set`/инстансы классов — непрозрачные листья (реактивность по замене ссылки). API помечен как экспериментальный и может измениться до стабилизации.
...
- `Computed` дедуплицировал свой выходной поток (`obs`) строгим `===` вместо `Object.is`, в обход фикса `State.set`: `Computed(() => NaN)` эмитил начальное значение дважды (пайплайн структурно дублирует первый emit, а `NaN === NaN` — `false`, поэтому дубль не схлопывался), а реальная смена `+0` → `-0` при пересчёте молча терялась. Теперь сравнение в `distinctUntilChanged` идёт через `Object.is` — согласованно со `State.set` и `ComputeCache`.
...
- `reduxDevtools()` без явного `driver` падал `ReferenceError: window is not defined` в SSR / Node-окружении: код читал `window.__REDUX_DEVTOOLS_EXTENSION__` напрямую, а обращение к необъявленному `window` бросает ещё до любого оператора (ни `?.`, ни `try/catch` вокруг чтения не спасают). Теперь доступ защищён проверкой `typeof window`: при отсутствии `window` (расширения там всё равно нет) бросается штатная ошибка «Redux Devtools extension is not installed».

### Changed
- `unstable_ProxySignal`: удержание путей сделано точным — материализованный узел считает реактивных подписчиков и, потеряв последнего, отсекается отложенно (микротаском). Долгоживущие сторы, читающие много разных динамических путей, больше не удерживают «холодные» узлы бесконечно; корректность не меняется — дремлющие `Computed` по-прежнему читают живые значения.
