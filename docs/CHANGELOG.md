...
### Added
- **`placeholderData`** — опция ресурса: `(args, previous) => { data } | null`. Пока для этих args ничего не закэшировано, сцепление показывает возвращённые данные как `dataSource: "placeholder"`; в кэш они не попадают и в `resource.getState()` не видны. Опция вызывается один раз на ключ args — retry, инвалидация и повторный пересчёт состояния переиспользуют первый ответ, а при попадании в кэш она не вызывается вовсе. Второй аргумент `previous` — снимок SWR-данных предыдущих args на момент первого вызова: фоновое обновление предыдущей записи результат не пересчитывает. Плейсхолдер важнее previous; чтобы оставить обратный приоритет, верните `null`, когда `previous` не пуст. Проекционные ресурсы (и, как следствие, страницы `useInfiniteResource`) опцию пока не поддерживают. См. [docs/query/api/resource](./query/api/resource.md#placeholderdata).
...
### Changed
- 💥 **Breaking. Состояние сцепления ресурса — матрица `status` × `dataSource`.** `status` теперь отвечает только на вопрос «идёт ли запрос и чем кончился прошлый»: `idle | pending | success | error`. Что показано на экране, говорит новое поле `dataSource`: `none | placeholder | previous | current` — единственный источник истины о наличии данных, потому что `TData` сам может быть `null`. Приоритет показа: `current` → `placeholder` → `previous` → `none`. Инвалидация в полёте стала `status: "pending"` с `dataSource: "current"`, упавшая инвалидация — `status: "error"` с `dataSource: "current"`, SWR при смене args — `status: "pending"` с `dataSource: "previous"`. Флаги производные: `isPending`, `isInitialLoading`, `isSwitching`, `isInvalidating`, `hasData`, `hasError`; пока запрос в полёте, истинен ровно один из `isInitialLoading` / `isSwitching` / `isInvalidating`, а повтор в полёте — это `isPending && hasError`. Объединение перечисляет 14 строк матрицы, а не свободное произведение, так что сужение работает по любой оси. `TClutchStatus` — четыре значения. Практическое следствие: данные надо рендерить по `hasData`, а не по `status === "success"`, иначе каждая инвалидация мигает спиннером поверх готовых данных. См. [docs/query/api/resource-clutch](./query/api/resource-clutch.md).
- 💥 **Breaking. Состояние сцепления команды.** `isLoading` / `isSuccess` / `isError` заменены на `isPending` / `hasData` / `hasError`. В `pending` данных больше нет вообще (`data: null` по типу): повторный `trigger()` с тем же ключом создаёт новую запись, переносить нечего. `hasError` в `pending` означает повтор и держит повторяемую ошибку читаемой. Единственный метод — `retry()`. См. [docs/query/api/command-clutch](./query/api/command-clutch.md).
- 💥 **Breaking. Флаги `useInfiniteResource`.** `isLoading` → `isPending`, `isFetchingNext` → `isLoadingNext`, `isError` → `hasError`; добавлены `isInvalidating` и `hasData`. Три флага загрузки — не разбиение `isPending`, а «есть хоть одна такая страница»: после инвалидации ленты с упавшей последней страницей `isInvalidating` и `isLoadingNext` истинны одновременно. В `data` попадают только страницы с данными своих args (`dataSource: "current"`) — раньше все с `data !== null`. См. [docs/query/usage/projection-resource](./query/usage/projection-resource.md).
- 💥 **Breaking. `useSuspenseResource` отдаёт всё, что есть что показать.** Порядок: `hasData` → вернуть состояние; `status === "error"` → бросить ошибку в Error Boundary; иначе suspend. Ошибка поверх previous- или placeholder-данных теперь возвращается, а не бросается. Возвращаемый тип — состояние сцепления, сужённое по `dataSource: "placeholder" | "previous" | "current"`. См. [docs/usage/react](./usage/react/README.md).
- 💥 **Breaking. `invalidate()` работает из ошибки.** В машине появился переход `error → pending` с обнулением ошибки, и `entry.invalidate()` его разрешает — он нужен рёбрам матрицы «ошибка поверх previous/placeholder → перезапрос». Побочный эффект на публичном API: `resource.invalidate(args)` на упавшей записи теперь перезапускает запрос и снимает ошибку, а не пишет предупреждение и ничего не делает (ошибку сохраняет `getEntry(args)?.retry()`). Сцепление при этом продолжает отклонять `invalidate()` там, где показывать нечего (строка 7 матрицы): `console.warn` и no-op. `entry.invalidate()` на записи команды — всегда `console.warn` и no-op, поэтому `invalidating` / `invalidate-error` для команды недостижимы. См. [docs/query/concepts/machine](./query/concepts/machine.md).
- 💥 **Breaking. Формат состояний машины.** Из `TPendingState` / `TInvalidatingState` убрано поле `isRetrying`: повтор в полёте — это `error !== null`, второй источник истины не нужен.
...
- 💥 **Breaking. Состояние записи: `IResourceLiteState` → `TResourceEntryState`, форма — по матрице.** `resource.getState(args)` возвращает состояние сцепления одной записи: те же поля и флаги, `dataSource` сужен до `none | current` (у записи нет ни previous, ни плейсхолдера), методов нет. Строки матрицы — 1, 2, 5, 6, 7, 9, 10, 12; `isSwitching` там всегда `false`. Сам вызов не изменился. См. [docs/query/api/resource](./query/api/resource.md#getstate).
...
### Removed
- флаги `isLoading`, `isRefreshing`, `isRefreshError`, `isRetrying`, `isSuccess`, `isError` — из состояния сцепления ресурса и из `resource.getState()`; `isLoading`, `isSuccess`, `isError` — из состояния команды; `isLoading`, `isFetchingNext`, `isError` — из `useInfiniteResource`. Замены — в таблицах [docs/query/api/resource-clutch](./query/api/resource-clutch.md);
- статусы `invalidating` / `invalidate-error` — из состояния сцепления и записи (в машине они остаются);
- типы `TRetrying`, `TSuspenseResourceErrorState`, `TResourceClutchInvalidatingState`, `TResourceClutchInvalidateErrorState`, `TResourceEntryInvalidatingState`, `TResourceEntryInvalidateErrorState` — вместе с формами, которые они описывали.
