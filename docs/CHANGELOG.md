...
- **`useInfiniteResource`** — React-хук бесконечной подгрузки, доступный **только на batch-ресурсах** (через `reactHooksPlugin()`): лента как список страниц-записей (`fetchNext` / `refresh` / `reset`, склеенный `data`), без миганий загруженных страниц, с дедупликацией элементов и живыми обновлениями через кэш элементов. В протокол плагинов добавлены `augmentBatchResource` и HKT-слот `batchResourceType`. См. [docs/query/usage/batch-resource](./query/usage/batch-resource.md#бесконечная-лента-useinfiniteresource).
