...
- `driver` — драйвер для хранения (по умолчанию localStorage, можно заменить на кастомный драйвер; для автоочистки драйверу нужно перечисление ключей — метод `keys(): string[]` либо пара `length`/`key(i)`)
- `gc` — политика автоочистки слота: `false` (или `{ enabled: false }`) — слот никогда не удаляется автоматически; `{ maxUnreadTime }` — своё время жизни в мс. По умолчанию `true` (60 дней)

#### Хранение и автоочистка

Каждое значение лежит в отдельном ключе хранилища, в конверте с метаданными:

```
__LSValue__                  → { v, nextGcAt }     — мета неймспейса: версия формата, план автоочистки
__LSValue__:{key}            → { at, ttl?, data }  — common-значение
__LSValue__:{key}:user:{id}  → { at, ttl?, data }  — значение конкретного пользователя
```

- **Изоляция слотов.** Повреждённая запись (битый JSON, чужая структура) не влияет на соседние слоты: она игнорируется с откатом к `defaultValue` и удаляется (self-heal). Запись слота — один атомарный `setItem`, вкладки не затирают чужие слоты.
- **Версия формата.** Если мета-ключ `__LSValue__` отсутствует или несёт другую версию — весь неймспейс `__LSValue__*` очищается и размечается заново (миграций между версиями формата нет). Более новую версию (соседняя вкладка с новой версией пакета) библиотека не трогает.
- **Автоочистка.** Примерно раз в `checkInterval` (по умолчанию неделя) удаляются слоты, которые не читались и не писались дольше их `maxUnreadTime` (по умолчанию 60 дней) — например, данные давно разлогиненных пользователей. Чтение обновляет отметку `at` (не чаще раза в `checkInterval`), поэтому живые значения не протухают. Вместо межвкладочных блокировок — случайное смещение запуска (`randomOffset`, до часа; применяется и к просроченному при старте дедлайну, чтобы не тормозить инициализацию) и перепроверка дедлайна в мете перед началом. Ключи обрабатываются порциями по `syncLimit` (20), между порциями поток отпускается.
- **Глобальные настройки** — `LocalSignal.GC_OPTIONS` (`syncLimit` / `checkInterval` / `randomOffset`, всё в мс). Дефолты, включая `maxUnreadTime`, — в экспортируемой константе `LOCAL_STATE_GC_DEFAULTS`.
