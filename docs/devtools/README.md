...
| `invalidate`    | Начат фоновый SWR-перезапрос (`invalidate()`, `prefetch(args, { force: true })`) |
| `revalidate`    | Начат отложенный перезапрос [помеченной записи](../query/concepts/cache.md#инвалидация-тающей-записи): при первом удержании или когда завершился запрос, которому инвалидация под `trail` дала доработать (у стрима — его завершение) |
| `rebase`        | Пришли данные фонового перезапроса (активные патчи переигрываются поверх них) |
| `invalidate-error` | Фоновый перезапрос упал — прежние данные остались                     |
...
UPDATE: invalidate, patch       две записи в одном батче
