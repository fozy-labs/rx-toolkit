# Оптимистичные обновления (Query v2)
...
`useResourceV2Agent` возвращает `entry` — handle кэш-записи для оптимистичных патчей:
...
const todosResource = api.createResourceV2<void, { items: Todo[] }>({
    key: 'todos',
