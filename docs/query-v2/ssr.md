...
| `createResourceV2` | — | Переопределяет API-уровень |
...
const frequentResource = api.createResourceV2({
    key: 'prices',
    queryFn: fetchPrices,
...
export async function getServerSideProps() {
    const api = unstable_queryV2.createApi({ keyPrefix: 'app' });
    const userResource = api.createResourceV2<{ id: string }, User>({
        key: 'users',
        queryFn: fetchUser,
    });
...
export async function render() {
    const api = unstable_queryV2.createApi({ keyPrefix: 'vite-app' });
    const resource = api.createResourceV2({ key: 'data', queryFn: fetchData });
