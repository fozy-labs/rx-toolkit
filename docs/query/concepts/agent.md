...
Статус `refresh-error` — это полноценное состояние [машины][machine] (переход `refreshing → refresh-error` при ошибке фонового обновления). Агент транслирует его напрямую, без трансформации.
...
| `refreshing` | `refreshing` | stale `TData` | Фоновое обновление, показываются устаревшие данные |
| `refresh-error` | `refresh-error` | stale `TData` | Обновление завершилось ошибкой, стейл данные сохраняются |
