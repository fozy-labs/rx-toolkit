...
graph TD
    ...
    HOOKS -- "создаёт" --> CLUTCH
    CLUTCH -- "наблюдает" --> ENTRY
```
...
- **Карта кэша** — реактивная коллекция записей (по одной на ресурс/команду), индексированных строковым ключом. Добавление и удаление записи точечно будит только зависящих читателей, не затрагивая остальных.

- **CacheEntry** — публикует состояние через Signal + RxJS Observable. При отписке всех подписчиков запускается таймер (`retentionTime`, по умолчанию 60 с для ресурсов), после которого запись удаляется из карты кэша.

### Ядро

- **Состояние записи** — иммутабельно: каждый переход возвращает новую запись состояния. Патчинг (оптимистичные обновления через Immer) выполняется на уровне записи кэша. Подробнее — [query-entry-state.md][entry-state].
- **Clutch** — SWR-наблюдатель, связывающий потребителя с записью кэша. Создаётся как хуками (`useResource`, `useCommand`), так и программно (`createClutch()`). При смене аргументов сохраняет предыдущие данные как fallback. Подробнее — [clutch.md][clutch].
...
- **useResource / useCommand** — React-хуки, создающие [сцепление][clutch] и подписывающиеся на его состояние.
...
- **SyncDriver** — абстракция кросс-табовой синхронизации кэша. Работает по PULL-модели: перед выполнением `queryFn` ресурс вызывает внутренний хук `beforeQuery`, инжектированный `createApi` при создании ресурса. Через `beforeQuery` ресурс запрашивает данные у других табов (REQ). Если другой таб отвечает (RES), запись гидрируется в success-состояние и `queryFn` не вызывается. Если ответа нет — `queryFn` выполняется штатно. `SyncDriver` не связан с картой кэша или записями кэша напрямую. Встроенная реализация — `broadcastSyncDriver` на основе BroadcastChannel API. Подробнее — [broadcast.md][usage-broadcast].
...
| **Состояние записи** (TQueryEntryState) | Иммутабельная запись состояния одного запроса: статус, данные, ошибка, метаданные. См. [query-entry-state.md][entry-state] |
| **Запись кэша** (CacheEntry) | Реактивный контейнер, хранящий одно такое состояние. GC по retention-таймеру. См. [cache.md][cache] |
| **Карта кэша** | Реактивная коллекция записей, индексированных ключом (сериализация или ссылочное сравнение аргументов) |
| **Сцепление** (Clutch) | SWR-наблюдатель, связывающий UI-компонент с записью кэша. См. [clutch.md][clutch] |
...
| **Связь** (Link) | Декларативная связь между ресурсом/командой: инвалидация (`invalidate`), оптимистичное обновление. См. [links.md][usage-links] |
| **Плагин** (Plugin) | Расширение, добавляющее методы на ресурс/команду при создании через API. См. [plugins.md][usage-plugins] |
| **`SKIP`** | Специальный символ, который передаётся вместо аргументов, чтобы отложить запрос. Сцепление переходит в `idle` |
...
| **Keyed-аргументы** (keyedArgs) | Аргументы, обёрнутые в `TKeyed<TArgs>` — пара `{ value, key }` с предвычисленным ключом кэша. См. [keyed.md][keyed] |
...
| **SyncDriver** | Абстракция кросс-табовой синхронизации (PULL-модель через `beforeQuery`). Не связан с картой кэша или записями напрямую. Встроенная реализация — `broadcastSyncDriver`. См. [broadcast.md][usage-broadcast] |

## См. также

- [Потоки данных][dataflows] — подробные sequence-диаграммы всех сценариев (cache miss/hit, инвалидация, мутации)
- [Кэш][cache] — устройство карты кэша и CacheEntry, retention-таймер, GC
- [Состояние записи запроса][entry-state] — пять статусов, переходы и патчинг
- [Сцепление][clutch] — SWR-наблюдатель, fallback-данные, подписки
