...
- **Оптимистичные патчи — set-local.** `createPatch` (напрямую или через `links` команд) применяется к проекции конкретного набора: оптимистика и ребейс внутри него работают штатно, но кэш элементов и пересекающиеся наборы патч не видят — они останутся прежними до своего рефреша. При первом патче на batch-ресурсе выводится однократное предупреждение в консоль.
