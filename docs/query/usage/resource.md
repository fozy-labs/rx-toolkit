...
Ресурс — абстракция для **чтения данных** с автоматическим кешированием и stale-while-revalidate (SWR). Для операций записи используйте [команду][command].
...
Запускает повторный запрос для кеш-записи. Если у записи есть активные подписчики — запрос выполняется немедленно. Без подписчиков — данные будут перезапрошены при следующем обращении.


### getEntry

Синхронно возвращает кеш-запись для указанных аргументов, или `null` если данные ещё не запрашивались. С флагом `doInitiate = true` — создаёт запись и запускает загрузку, если её ещё нет.
