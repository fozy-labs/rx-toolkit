...
`queryFn` выводится из тела функции, аннотируйте его явно
(`queryFn: async (id: number): Promise<User> => ...`) — либо аннотируйте `ctx`
в хуке, либо передайте дженерики явно: `api.createResource<TArgs, TData>({ ... })`.
