...
- **`invalidate()` / `retry()` / `fetch()`** отписываются от текущего стрима и подписываются заново (`invalidate` для стрима = переподписка). Первая эмиссия нового запуска проходит через ребейз (`invalidating → success`).
...
- **Снимки (SSR).** Сериализуется последняя эмиссия success-записи, как у обычного ресурса. Гидрированная запись **не** переподключает стрим сама — данные статичны до первой `invalidate()` или перезапуска записи. Для чисто «живых» ресурсов рассмотрите `snapshotable: false`.
