...
| Кто перезапрашивает | `invalidate()` / SWR — фоновый перезапрос | `retry()` — повтор упавшей мутации |
...
- Новый `execute` / `trigger` создаёт новую запись → **новый** request id (это другая логическая операция).
...
> Request id — это не [ключ кэша][cache]. Ключ кэша адресует запись внутри toolkit (состояние, сцепление, retention);
