...
Снимок (snapshot) позволяет сериализовать состояние кеша на сервере и передать его клиенту, чтобы избежать повторных запросов при гидрации. Механизм работает как для [ресурсов][resource], так и для [команд][command].
...
Гидрация происходит **лениво**: при вызове `createResource()` библиотека ищет соответствующий slice в снимке и восстанавливает кеш-записи. После гидрации slice удаляется из внутреннего хранилища (consume-паттерн). Вызов `resetAll()` обнуляет сохранённый снимок целиком.
...
Инвалидация запускает рефетч — компонент получит свежие данные без дополнительного кода.


## Жизненный цикл

1. **Сервер** — `api.getSnapshot()` сериализует успешные записи кеша в `TApiSnapshot`.
...
4. **Гидрация ресурсов** — каждый `createResource()` достаёт свой slice и восстанавливает записи в [кеш][cache].
5. **Авто-инвалидация** — если задан `snapshotValidTime` и запись устарела, она инвалидируется с рефетчем.
...
- [Кеш][cache] — управление кеш-записями и жизненный цикл GC
