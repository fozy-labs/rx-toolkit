...
| `retentionTime`      | `number \| false \| ((state: TState) => number \| false)` | (Обязательное поле) | Время (мс) удержания записи после отписки последнего подписчика. `false` — не удалять. Функция вызывается на каждом переходе `active → retention` и получает состояние самой записи на этот момент; ловля броска, [нормализация результата][cache-normalize] и таймер — здесь же. |
