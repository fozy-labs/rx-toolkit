...
- **Прерванные запросы не мапятся.** Отменённый ран — это управление потоком, а не провал запроса; он не доходит до машины и не попадает в `mapError`. Реджекты `ensure()` / `fetch()` при удалении записи также остаются сырым `CacheEntryRemovedError` — эти каналы не типизированы `TError`.
...
- **Дискриминированные состояния.** Состояния ресурсов и команд — дискриминированные объединения: `state.isError` сужает `error` до `TError` (без `| null`), `state.isSuccess` — `data` до `TData`. См. [сцепление ресурса](./resource-clutch.md#состояние-tresourceclutchstate) и [сцепление команды](./command-clutch.md#состояние-tcommandclutchstate).
