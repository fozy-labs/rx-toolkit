...
| `onCacheEntryAdded`  | `TLifecycleHookOption<(args, ctx) => void>` | —                 | Хук жизненного цикла уровня API — вызывается при создании любой кэш-записи. Принимает один хук или их массив. См. [lifecycle hooks][lifecycle]. |
| `onQueryStarted`     | `TLifecycleHookOption<(args, ctx) => void \| Promise<void>>` | — | Хук жизненного цикла уровня API — вызывается при каждом запуске `queryFn`. Принимает один хук или их массив. См. [lifecycle hooks][lifecycle]. |
...
| `unstable_createProjectionResource` | `options: TProjectionResourceOptions<...>` | `IApiResource<TPlugins, TArgs, TItem[]>` | Создаёт [проекционный ресурс][проекционный ресурс] — обёртку над ресурсом с кэшем на уровне элементов. |
...
По умолчанию поле `error` в состояниях ресурсов и команд имеет тип `unknown` — так же, как ловится любая ошибка в JS. Опция `mapError` уровня API позволяет привести любую сырую ошибку к единому типизированному виду. Функция вызывается **ровно один раз** на каждый провал — в тот момент, когда ошибка входит в состояние записи кэша, — поэтому один и тот же нормализованный экземпляр видят все потребители: состояние сцепления/хука, реджект `ensure()` / `fetch()`, бросок в Error Boundary у Suspense-хука и конверт `TTriggerResult` у мутаций. Тип, который возвращает `mapError`, автоматически выводится в `TError`.
...
- **Один вызов, один экземпляр.** Маппинг происходит на единственной границе — в точке, где провал входит в состояние записи кэша; downstream все видят один и тот же экземпляр.
- **Ошибки жизненного цикла мутаций тоже мапятся.** Если кэш-запись команды удаляется до завершения мутации (повторный `execute` с тем же ключом, `reset()` / `resetAll()`), реджект `Command.execute` и конверт `TTriggerResult` получают `CacheEntryRemovedError`, пропущенный через `mapError`, — типизация `TError` не нарушается. Класс `CacheEntryRemovedError` экспортируется публично: закладывайте в маппер ветку для него (или общий fallback для неизвестных ошибок).
- **Прерванные запросы не мапятся.** Отменённый ран — это управление потоком, а не провал запроса; он не доходит до состояния записи и не попадает в `mapError`. Реджекты `ensure()` / `fetch()` при удалении записи также остаются сырым `CacheEntryRemovedError` — эти каналы не типизированы `TError`.
- **Бросок в `mapError` не ломает запись.** Если сам `mapError` бросит — падение логируется в `console.error`, а в состояние идёт исходная (сырая) ошибка.
- **Дискриминированные состояния.** Состояния ресурсов и команд — дискриминированные объединения: `state.hasError` сужает `error` до `TError` (без `| null`), `state.hasData` — `data` до `TData`. См. [сцепление ресурса](./resource-clutch.md#состояние-tresourceclutchstate) и [сцепление команды](./command-clutch.md#состояние-tcommandclutchstate).
...
- [Проекционный ресурс](../usage/projection-resource.md)
