...
| `invalidate` | `() => void` | Перезапрашивает текущие args и снимает ошибку: строки 5 → 6, 8 → 4, 9 → 6, 13 → 3. Показанные данные остаются на экране, при неудаче сохраняются. Вне этих рёбер — `console.warn` и no-op: и во время запроса в полёте, и в строке 7, где перепроверять нечего (там нужен `retry()`). |
...
`retry()` повторяет упавший запрос и **показывает ошибку** дальше; `invalidate()` перепроверяет показанное и **снимает** её. Оба перезапрашивают **текущие** args — в строке 8 те, что упали, а не те, чьи данные на экране. Вызов вне нарисованных рёбер — `console.warn` и no-op, как у [записи кэша][cache].
...
- `status === 'pending'` ⇔ запрос в полёте. Для сужения `data` / `error` в строках 6, 10–12 и 14 дискриминант не `status`, а `dataSource` × `hasError`.
